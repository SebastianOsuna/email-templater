...
module.exports = function(directory){
     ...
     */
     var templateExists = function ( template_name, format, _then, _else ) {
        var template_path = [];
        if( !_else && format instanceof Function ) {
            _else = _then;
            _then = format;
            template_path.push( _dir + "/" + template_name + "." + "txt" );
            template_path.push( _dir + "/" + template_name + "." + "html" );
        } else {
            template_path.push( _dir + "/" + template_name + "." + format );
        }

        fs.exists( template_path[0], function ( exists ) {
            if ( exists ) {
                _then( template_path[0] );
            } else if ( template_path[1] ) {
                fs.exists( template_path[1], function ( exists ) {
                    if ( exists ) {
                        _then( template_path[1] );
                    } else if ( _else ) {
                        _else();
                    }
                } );
