 ...
 * @param templates_directory Template's directory
 ...
 * @param error_cb Error callback
 */
function edit_template (name, templates_directory, body, success_cb, error_cb) {
    var exts = Object.keys( body ).filter( template_format_filter ), c = exts.length, e = 0;
    if (c > 0) {
        exts.forEach( function (ext) {
            var template = body[ext].template || body[ext];
 ...
 * @param error_cb Error callback
 */
function delete_template (name, directory, format, success_cb, error_cb) {
    var template = template_exists( name, directory, format );
    if( template ) {
